...
for i in range(len(grid)):  # looping the rows
    for j in range(len(grid[i])):  # looping the cols
        if i+3 <= len(grid)-1:
            ...
            if v > maxproduct: maxproduct = v
        if j+4 < len(grid[i])-1:
            h = grid[i][j]*grid[i][j+1]*grid[i][j+2]*grid[i][j+3]
            if h > maxproduct: maxproduct = h
        if i+3 <= len(grid[i])-1 and j+3 <= len(grid[i])-1:
            dr = grid[i][j]*grid[i+1][j+1]*grid[i+2][j+2]*grid[i+3][j+3]
            if dr > maxproduct: maxproduct = dr
        if j+3 <= len(grid[i])-1 and i-3 in range(3,20):
            dl = grid[i][j]*grid[i-1][j+1]*grid[i-2][j+2]*grid[i-3][j+3]
            if dl > maxproduct: maxproduct = dl
